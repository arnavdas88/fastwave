...
async def show_cyan_dashboard(q: Q):
    ...
    q.page["header"] = ui.header_card(
        box="header",
        title="FastWave Demo",
        subtitle="H2O Wave - Cyan Dashboard",
        image="https://wave.h2o.ai/img/h2o-logo.svg",
        items=[
            ui.tabs(name="Dashboards", value="#dashboards/cyan", items=global_nav),
        ],
    )
            ...
            ui.stats(
                items=[
                    ui.stat(
                        ...
                        caption=next(sample_dollars),
                    ),
                    ui.stat(
                        label=next(sample_term),
                        value=next(sample_dollars),
                        caption=next(sample_percent),
                    ),
                    ui.stat(
                        label=next(sample_term),
            ...
            ui.inline(
                items=[
                    ui.dropdown(
                        ...
                        value="option0",
                        choices=[
                            ui.choice(name=f"option{i}", label=next(sample_term))
                            for i in range(5)
                        ],
                    ),
                    ui.dropdown(
