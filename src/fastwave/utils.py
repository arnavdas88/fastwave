    ...
    with open(os.path.join(web_directory, "index.html"), "r") as f:
        for line in f:
            ...
            if re.search(r"(\.js|\.css)(\'|\")", stripped_line):
                web_files.append(
                    stripped_line.replace(find_str, f"/{prefix}wave-static/")
                )
    web_files = "\n".join(web_files)
...
def change_signature(original_function, new_parameters):
    ...
    new_params = [
        inspect.Parameter(
            name, inspect.Parameter.POSITIONAL_OR_KEYWORD, annotation=param_type
        )
...
def get_parameters(func: typing.Callable, exclude_types: typing.List[typing.Any] = []):
    ...
    allowed_parameters: typing.List[typing.Tuple[str, typing.Any]] = [
        (key, values.annotation)
        for (key, values) in func_parameters.items()
        if type_support_pydantic(values.annotation, exclude_types)
    ]
...
def takes_websocket_as_input_parameter(func: typing.Callable) -> bool:
    for param_name, _ in list(
        filter(lambda x: x[1] is WebSocket, func.__annotations__.items())
    ):
        return param_name
    return False
